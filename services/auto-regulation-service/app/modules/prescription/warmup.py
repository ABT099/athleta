...
class WarmupGenerator:
    ...
    def determine_warmup_set_count(
        ...
        if exercise.safety.complexity_score:
            if exercise.safety.complexity_score >= 1.5:
                complexity_adjustment = 1
            elif exercise.safety.complexity_score >= 1.2:
                complexity_adjustment = 0
            elif exercise.safety.complexity_score <= 0.8:
                complexity_adjustment = -1

        # Adjust based on injury risk (higher = more warm-up sets)
        # Injury risk typically ranges from 0.0 to 1.0
        injury_adjustment = 0
        if exercise.safety.injury_risk_level:
            if exercise.safety.injury_risk_level >= 0.7:
                injury_adjustment = 1
            elif exercise.safety.injury_risk_level >= 0.5:
                injury_adjustment = 0
            elif exercise.safety.injury_risk_level <= 0.3:
                injury_adjustment = -1
