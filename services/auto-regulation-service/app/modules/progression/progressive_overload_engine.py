...
from app.modules.periodization import PeriodizationService
...
class ProgressiveOverloadEngine:
    ...
    def _calculate_exercise_specific_adjustments(
        ...
        primary_muscle_map = {}
        if exercise_ids:
            # Load all exercises upfront from exercise-service
            with ExerciseClient() as client:
                exercises_list = client.get_exercises(exercise_ids)
            exercise_map = {ex.id: ex for ex in exercises_list}

            # Primary muscle = highest-priority role target (prime_mover first).
            role_priority = {"prime_mover": 1, "synergist": 2, "stabilizer": 3}
            for ex in exercises_list:
                best_priority = None
                for target in ex.muscles:
                    priority = role_priority.get(target.role, 4)
                    if best_priority is None or priority < best_priority:
                        best_priority = priority
                        primary_muscle_map[ex.id] = target.name
