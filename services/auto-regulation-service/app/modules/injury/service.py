...
from app.models import WorkoutSession, ExerciseSet, Athlete
...
class InjuryPreventionService:
    ...
    def _get_exercise(self, exercise_id: int):
        """Fetch a single exercise, or None if unknown."""
    ...
    def calculate_weighted_joint_stress(
        ...
        # Query recent training sets, then resolve their exercises over gRPC
        recent_sets = (
        ...
        exercises_map = self._get_exercises_map({s.exercise_id for s in recent_sets})

        joint_stress_scores: Dict[str, float] = {}

        for ex_set in recent_sets:
            exercise = exercises_map.get(ex_set.exercise_id)
    ...
    def check_joint_stress(
        ...
        high_risk_exercises = [
            ...
            if ex.safety.injury_risk_level >= 2.5
        ]
    ...
    def check_form_degradation(self, athlete_id: int, sessions_to_check: int = 3) -> Dict:
        ...
        exercises_map = self._get_exercises_map(exercise_analysis.keys())
        for exercise_id, ex_analysis in exercise_analysis.items():
            ...
            if exercise:
                joint_stress_areas = list(exercise.safety.joint_stress_areas)
        ...
        return {
            ...
            "exercise_analysis": {
                ex_id: {
                    "exercise_name": exercises_map[ex_id].name if ex_id in exercises_map else f"Exercise {ex_id}",
    ...
    def analyze_form_degradation_patterns(
        ...
        for issue in chronic_issues.get("issues", []):
            exercise = self._get_exercise(issue["exercise_id"])
            if not exercise:
                continue
    ...
    def _generate_exercise_specific_recommendation(
        ...
        joint_stress = list(exercise.safety.joint_stress_areas)
